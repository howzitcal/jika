...
# apt installs
sudo apt-get install -yq gnome-software-plugin-snap gnome-software-plugin-flatpak gnome-shell-extension-manager vlc tilix unzip breeze-cursor-theme ca-certificates gpg gnupg2 apt-transport-https gcc make curl
...
if [[ -v kooha ]]; then
    flatpak install --noninteractive -y io.github.seadve.Kooha
    dconf write /org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/custom0/binding "'<Shift><Super>r'"
    dconf write /org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/custom0/command "'flatpak run io.github.seadve.Kooha'"
    dconf write /org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/custom0/name "'kooha'"
